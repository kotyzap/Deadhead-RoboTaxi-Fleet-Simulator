# In-Car Design Language — reference notes

> **Provenance, and why this file is worded the way it is.** These are observation
> notes on the visual language of premium EV showroom and in-car interfaces
> generally — radical subtraction, one accent, no shadows, two weights. They were
> taken from public marketing sites while designing Deadhead's console, and in
> 0.42.0 they were rewritten to describe the *pattern* rather than any one
> company's site. Nothing here is anyone's intellectual property: hex values,
> spacing rhythms and "avoid box-shadows" are facts and opinions about layout,
> which is exactly the category §9.3 of DESIGN.md says is NOT protectable.
> Names, logos and typefaces are protectable, and none appear here.
>
> Replaces `TeslaDesignSystem.md` (deleted in 0.42.0). If you are reading this in
> a checkout that still has that file, delete it — see the 0.42.0 release note.

## 1. Visual Theme & Atmosphere

The reference showroom site is an exercise in radical subtraction — a digital showroom where the product is everything and the interface is almost nothing. The page opens with a full-viewport hero that fills the entire screen with cinematic car photography: three vehicles arranged on polished concrete against a hazy cityscape sky, with a single model name floating above in translucent white type. There are no decorative borders, no gradients, no patterns, no shadows. The UI exists only to provide just enough navigational structure to get out of the way. Every pixel that isn't product imagery is white space, and that restraint is the design system's most powerful statement.

The color philosophy is almost ascetic: a single blue (`#3E6AE1`) for primary calls to action, three shades of dark gray for text hierarchy, and white for everything else. The entire emotional weight is carried by photography — sprawling landscape shots, studio-lit vehicle profiles, and atmospheric environmental compositions that stretch edge-to-edge across each viewport-height section. The UI chrome dissolves into the imagery. The navigation bar floats above the hero with no visible background, border, or shadow — the THE BRAND wordmark and five navigation labels simply exist in the space, trusting the content beneath them to provide sufficient contrast.

The reference typography is a commercially licensed custom family split into "Display" for headlines and "Text" for body/UI elements — unifying the website, mobile app, and in-car software into a single typographic voice. The Display variant renders hero titles at 40px weight 500, while the Text variant handles everything from navigation (14px/500) to body copy (14px/400). The font carries a geometric precision with slightly humanist terminals that feels engineered rather than designed — exactly matching the brand's brand identity of technology that doesn't need to announce itself. There are no text shadows, no text gradients, no decorative type treatments. Every letterform earns its place through clarity alone.
...
- the licensed family font family (Display + Text) unifying web, app, and in-car interfaces
...
- No secondary accent colors exist. The pattern deliberately avoids color variety to maintain extreme visual discipline
...
### Semantic & Accent
- A marketing showroom avoids semantic color coding (no green/red/yellow status indicators). Error, success, and warning states follow standard browser defaults in form contexts
...
### Font Family
- **Display**: a geometric licensed display face, -apple-system, Arial, sans-serif — used for hero titles and large model names. A geometric sans-serif with precisely engineered proportions, chosen to unify the brand's digital ecosystem (website, mobile app, vehicle interface)
- **Text/UI**: `the licensed text face`, -apple-system, Arial, sans-serif — used for navigation, body copy, buttons, and all UI text. Optimized for legibility at smaller sizes with slightly wider proportions than the Display variant
...
|------|------|--------|-------------|----------------|-------|
| Hero Title | 40px (2.50rem) | 500 | 48px (1.20) | normal | the licensed family Display, white on dark hero imagery |
| Product Name | 17px (1.06rem) | 500 | 20px (1.18) | normal | the licensed text face, model names in nav panel and cards |
| Nav Item | 14px (0.88rem) | 500 | 16.8px (1.20) | normal | the licensed text face, primary navigation labels |
| Body Text | 14px (0.88rem) | 400 | 20px (1.43) | normal | the licensed text face, paragraph and descriptive content |
| Button Label | 14px (0.88rem) | 500 | 16.8px (1.20) | normal | the licensed text face, CTA button text |
...
### Principles
- **"Normal" letter-spacing everywhere**: Unlike most modern tech brands that use negative tracking for headlines, this pattern uses default letter-spacing at every level. This reflects a philosophy that the typeface should speak for itself without manipulation
...
- **No text transforms**: No uppercase text appears in the main navigation or CTAs — the lowercase approach reinforces the understated confidence
...
- Font: the licensed text face, 14px
- The "Ask a Question" chatbot input bar sits at the viewport bottom with a clean white background and subtle border

### Navigation
- **Desktop**: Centered horizontal nav with the brand wordmark (spaced uppercase letters) on the left, five category buttons center-aligned, and three icon buttons (help, globe/language, account) on the right
...
This pattern uses whitespace as a luxury signal. The generous vertical spacing between sections (each section is a full viewport height) means you can only see one "message" at a time — one car, one model name, one CTA pair. This creates a gallery-like browsing experience where each scroll is a deliberate transition, not a continuous feed. White space is not empty — it's the frame that elevates each vehicle to the status of art piece.
...
### Shadow Philosophy
The approach to elevation is essentially "none." The site avoids box-shadows entirely in its primary interface. Depth is communicated through three alternative strategies:
...
- Use uppercase text transforms — the confidence is expressed through lowercase calm
- Introduce rounded-pill buttons or large border-radii — the 4px radius is deliberate and precise
- Override the the licensed family family with other typefaces — cross-platform consistency is a core brand value
- Add hover animations with scale/translate transforms — the interactions are color-only (background and border transitions)
...
- "Create a hero section with a full-viewport background image, centered model-name title in the licensed family Display at 40px weight 500 in white, a subtitle line below, and two buttons side by side: a primary Electric Blue (#3E6AE1) 'Order Now' button and a secondary white 'View Inventory' button, both with 4px border-radius and 40px height"
